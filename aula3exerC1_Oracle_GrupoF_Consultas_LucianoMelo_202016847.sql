...
SELECT 
    ...
    s.NOME_SETOR,
    LISTAGG(e.NOME_ESPECIALIDADE, ', ') WITHIN GROUP (ORDER BY e.NOME_ESPECIALIDADE) AS ESPECIALIDADES_PLANTONISTA,
    TO_CHAR(h.HORA_INICIO, 'DD/MM/YYYY HH24:MI') AS INICIO_PLANTAO,
    TO_CHAR(h.HORA_FIM, 'DD/MM/YYYY HH24:MI') AS FIM_PLANTAO
FROM GF_PLANTONISTA p
INNER JOIN GF_ALOCACAO al ON p.MATRICULA = al.MATRICULA_PLANTONISTA
INNER JOIN GF_HORARIO h ON al.ID_HORARIO = h.ID_HORARIO
INNER JOIN GF_SETOR s ON al.ID_SETOR = s.ID_SETOR
LEFT JOIN GF_TEM t ON p.MATRICULA = t.MATRICULA_PLANTONISTA
LEFT JOIN GF_ESPECIALIDADE e ON t.ID_ESPECIALIDADE = e.ID_ESPECIALIDADE
WHERE 
    TRUNC(h.HORA_INICIO) BETWEEN TO_DATE(:DE_DD_MM_YYYY, 'DD/MM/YYYY') AND TO_DATE(:ATE_DD_MM_YYYY, 'DD/MM/YYYY')
GROUP BY 
    p.MATRICULA,
    p.NOME_COMPLETO,
    TO_CHAR(h.HORA_INICIO, 'DD/MM/YYYY HH24:MI'),
    h.HORA_FIM,
    s.NOME_SETOR
ORDER BY 
    TO_CHAR(h.HORA_INICIO, 'DD/MM/YYYY HH24:MI') ASC;
...
GROUP BY 
    ...
    a.NOME_ALA,
    a.QTDE_QUARTOS,
    a.QTDE_SALAS_CIRURGIA,
    a.QTDE_ENFERMARIAS,
    a.QTDE_PROFISSIONAIS
ORDER BY 
    p.MATRICULA;
...
-- além disso foi indtroduzido hints na criação da view e na consulta;
...
SELECT 
    ...
    CASE p.SEXO
        ...
        ELSE 'Outro'
    END AS SEXO_PLANTONISTA,
    e.NOME_ESPECIALIDADE AS ESPECIALIDADES_PLANTONISTA
FROM 
    GF_PLANTONISTA p
...
WHERE 
    h.HORA_FIM > TO_TIMESTAMP(:DIA || ' ' || :DE, 'DD/MM/YYYY HH24:MI')
    AND h.HORA_INICIO < TO_TIMESTAMP(:DIA || ' ' || :ATE, 'DD/MM/YYYY HH24:MI')
ORDER BY 
    h.HORA_INICIO;